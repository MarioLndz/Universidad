	...
	class GameUniverse
		...
		def combatGo (station, enemy)  # station es un SpaceStation y enemy un EnemyStartship
			...
			if (enemyWins)
				...
				if (@dice.robbedLoot)
					s = [(enemy.loot.nSupplies - @dice.robbedElements), 0].max
					w = [(enemy.loot.nWeapons - @dice.robbedElements), 0].max
					sh = [(enemy.loot.nShields - @dice.robbedElements), 0].max
					h = [(enemy.loot.nHangars - @dice.robbedElements), 0].max
					m = [(enemy.loot.nMedals - @dice.robbedElements), 0].max
					
					aLoot = Loot.new(s,w,sh,h,m)
				else
					aLoot = enemy.Loot
				end
